...
func (a *aapt) deps(ctx android.BottomUpMutatorContext, sdkDep sdkDep) {
	...
	if sdkDep.afterlifeResModule != "" {
		ctx.AddVariationDependencies(nil, afterlifeResTag, sdkDep.afterlifeResModule)
	}
}
...
func aaptLibs(ctx android.ModuleContext, sdkContext android.SdkContext, classLoaderContexts dexpreopt.ClassLoaderContextMap) (
	...
	ctx.VisitDirectDeps(func(module android.Module) {
		...
		case frameworkResTag, afterlifeResTag:
			if exportPackage != nil {
				sharedLibs = append(sharedLibs, exportPackage)
			}
		case staticLibTag:
...
func (a *AARImport) DepsMutator(ctx android.BottomUpMutatorContext) {
	if !ctx.Config().AlwaysUsePrebuiltSdks() {
		...
		if sdkDep.useModule && sdkDep.afterlifeResModule != "" {
			ctx.AddVariationDependencies(nil, afterlifeResTag, sdkDep.afterlifeResModule)
		}
	}
